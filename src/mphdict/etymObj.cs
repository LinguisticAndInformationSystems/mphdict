...
namespace mphdict
{
    public class etymObj : IDisposable
    {
        ...
        public async Task<main> getEntry(int idset)
        {
            try
            {
                var ss = await (from c in db.main.Include(o=>o._pofs).Include(o=>o._wlist).AsNoTracking() where c.id== idset select c).FirstOrDefaultAsync();
