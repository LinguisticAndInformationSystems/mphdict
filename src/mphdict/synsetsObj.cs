...
{
    public partial class synsetsContext
    {
        ...
        public List<ps[]> Pofs
        {
            get
            {
                if (_Pofs == null)
                {
                    ...
                    {
                        _Pofs = new List<ps[]>();
                        var g1 = (from c in this.pofs /*where (c.id <= 16)*/ orderby c.name select c).ToArray();
                        //var g2 = (from c in this.pofs where (c.id > 16) orderby c.name select c).ToArray();
        ...
        {
            var ss = await (from c in this.synsets.Include(o => o._pofs).Include(o => o._wlist).AsNoTracking() where c.id == idset select c).FirstOrDefaultAsync();
            ss._wlist = ss._wlist.OrderBy(o => o.id_syn).ToArray();
            return ss;
        }
        public async Task<string> getWord(int wid)
        {
            return await (from c in this.wlist where c.id == wid select c.word).FirstOrDefaultAsync();
        }
